...
import com.petshop.dto.OrderResponseDto;
import com.petshop.dto.UpdateDeliveryStatusRequest;
import com.petshop.entity.Cart;
import com.petshop.entity.Orders;
import com.petshop.entity.Product;
import com.petshop.entity.User;
import com.petshop.exception.OrderSaveFailedException;
...
import com.petshop.utility.Constants.DeliveryStatus;
import com.petshop.utility.Constants.DeliveryTime;
import com.petshop.utility.Helper;
...
public class OrderResource {
	...
	public ResponseEntity<CommonApiResponse> orderProductsFromCart(int userId) {
		...
		String mailBody = this.makeEmailBody(addedOrders);
		
		if(StringUtils.isEmpty(mailBody)) {
			LOG.error("Failed to send Order Mail to "+user.getEmailId());
		}
		
		boolean flag = this.sendOrderEmail(user.getEmailId(), "Online Pet Shop - Order Confirmation", mailBody);
		
		if(flag) {
			LOG.info("Order Confirmation Mail Sent to "+user.getEmailId());
		} else {
			LOG.error("Failed to send Order Mail to "+user.getEmailId());
		}
	...
	public static String makeEmailBody(List<Orders> orders) {
        StringBuilder emailBody = new StringBuilder();
        emailBody.append("<html><body>");
        emailBody.append("<h2>Dear ").append(orders.get(0).getUser().getFirstName()).append(",</h2>");
        emailBody.append("<p>Thank you for your order. Below is the order confirmation:</p>");

        // Create a dynamic table for the list of orders
        emailBody.append("<table border='1'>");
        emailBody.append("<tr><th>Sr No.</th><th>Product Name</th><th>Quantity</th><th>Price per Product</th></tr>");
        ...
        for (Orders order : orders) {
            emailBody.append("<tr>");
            emailBody.append("<td>").append(serialNumber++).append("</td>");
            emailBody.append("<td>").append(order.getProduct().getName()).append("</td>");
            emailBody.append("<td>").append(order.getQuantity()).append("</td>");
            emailBody.append("<td>").append(order.getProduct().getPrice()).append("</td>");
            emailBody.append("</tr>");

            // Calculate total amount
            totalAmount = totalAmount.add(order.getProduct().getPrice().multiply(BigDecimal.valueOf(order.getQuantity())));
        }

        emailBody.append("</table>");

        // Display the total amount
        emailBody.append("<p>Total Amount: Rs ").append(totalAmount).append("</p>");

        // Add "Regards" message
        emailBody.append("<p>Regards,<br>Online Pet Shop</p>");
